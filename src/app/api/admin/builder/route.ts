...
export async function GET(request: Request) {
  ...
  const siteId = searchParams.get("site_id") || "HOMCOT";
  ...
  try {
    ...
    await query(`
      create table if not exists builder_pages (
        ...
        site_id varchar(50) not null default 'HOMCOT',
...
export async function POST(request: Request) {
  ...
  const { schema, site_id } = body.schema ? { schema: body.schema, site_id: body.site_id } : { schema: body, site_id: "HOMCOT" };
  const siteId = site_id || "HOMCOT";
