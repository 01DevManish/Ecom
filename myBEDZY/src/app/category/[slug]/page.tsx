...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: `${category.description} Shop ${category.name.toLowerCase()} products online at myBEDZY with curated styles for Indian homes.`,
    alternates: {
      canonical: `/category/${category.slug}`,
    },
    openGraph: {
      title: `Buy ${category.name} Online | myBEDZY`,
  ...
  return (
    <section className="qh-container qh-section-pad">
      ...
      <article className="qh-seo-copy mt-10 rounded-lg border border-border bg-background-elevated p-6">
        ...
        <p>{category.description} myBEDZY keeps browsing simple with mobile-friendly product cards, clear prices, wishlist saving and quick cart actions.</p>
      </article>
    </section>
  );
}
